...
(function () {
    ...
    function TestClientViewModel(data) {
        ...
        self.RequestMediaType.subscribe(function () {
            ...
            //Appverse# Modification in the Simple Test Client for ASP.NET Web API to add automatically in the header (when it is available) the X-XSRF-Token
            var csrfToken = $("input[name='__RequestVerificationToken']").val();
            if (csrfToken != "" && typeof csrfToken !== "undefined") {
                addOrReplaceHeader(headers, "X-XSRF-Token", csrfToken);
                //httpRequest.setRequestHeader("X-XSRF-Token", csrfToken);
            }
        });
...
function addOrReplaceHeader(headers, headerName, headerValue) {
    ...
    for (var i in headerList) {
        // Appversere# modification: Added toLowerCase() to avoid including the X-XSRF-Token multiple times
        if (headerList[i].name.toLowerCase() == headerName.toLowerCase()) {
